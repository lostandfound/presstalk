...
class TestFasterWhisperEngine(unittest.TestCase):
    def test_flow(self):
        backend = FakeBackend()
        eng = FasterWhisperEngine(sample_rate=16000, language='ja', model='small', backend=backend)
    ...
    def test_backend_exception_returns_empty(self):
        ...
        eng = FasterWhisperEngine(sample_rate=16000, language='ja', model='small', backend=BadBackend())
        sid = eng.start_session()
    ...
    def test_timeout_returns_empty(self):
        ...
        eng = FasterWhisperEngine(sample_rate=16000, language='ja', model='small', backend=SlowBackend())
        sid = eng.start_session()
