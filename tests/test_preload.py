...
class TestModelPreloading(unittest.TestCase):
    ...
    @patch("faster_whisper.WhisperModel")
    def test_transcribe_is_fast_after_initialization(
        self, mock_whisper, mock_frombuffer
    ):
        ...
        result = backend.transcribe(
            b"dummy_audio", sample_rate=16000, language="en", model="tiny"
        )
    ...
    def test_progress_display_during_loading(self, mock_whisper, mock_print):
        ...
        _backend = FasterWhisperBackend(model="small", show_progress=True)
