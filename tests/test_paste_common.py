...
class TestPasteCommon(unittest.TestCase):
    ...
    def test_guard_disabled_returns_false(self):
        fg = {"name": "Terminal"}
        self.assertFalse(
            PasteGuard.should_block(
                fg, guard_enabled=False, default_blocklist="terminal"
            )
        )
    ...
    def test_env_guard_disables(self):
        ...
        self.assertFalse(
            PasteGuard.should_block(
                fg, guard_enabled=None, default_blocklist="terminal"
            )
        )
    ...
    def test_param_blocklist_takes_precedence(self):
        ...
        self.assertTrue(
            PasteGuard.should_block(fg, blocklist=["Terminal"], default_blocklist="")
        )
    ...
    def test_env_blocklist_when_param_missing(self):
        ...
        fg = {"name": "Terminal"}
        self.assertTrue(
            PasteGuard.should_block(fg, blocklist=None, default_blocklist="")
        )
    ...
    def test_default_blocklist_when_no_param_or_env(self):
        ...
        self.assertTrue(
            PasteGuard.should_block(
                fg, blocklist=None, default_blocklist="Terminal,iTerm2"
            )
        )
    ...
    def test_string_blocklist_splits_and_normalizes(self):
        ...
        self.assertTrue(
            PasteGuard.should_block(
                fg,
                blocklist=" powershell.exe , WindowsTerminal.exe ",
                default_blocklist="",
            )
        )
    ...
    def test_no_fg_info_never_blocks(self):
        self.assertFalse(
            PasteGuard.should_block(
                {}, blocklist=["anything"], default_blocklist="fallback"
            )
        )
