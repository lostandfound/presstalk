...
初期結論では「2キー限定」の制約下で Shift + Space を最有力としましたが、後続の実機検証により、テキスト入力可能なコンテキストで「押下中にスペース文字が継続的に挿入される」副作用が確認されました（Spaceが主キーのため）。Push-to-Talk運用では、入力フォーカスがある環境での誤入力は重大なUX劣化・データ破壊要因となるため、2キー要件を例外的に緩和し、最終推奨デフォルトを次の通り更新します。

最終推奨デフォルト: Ctrl + Shift + Space（3キー）

- 文字挿入の副作用が発生しない（Space単独入力を抑止）
- スクリーンリーダーと非競合でWCAG 2.1の意図に整合
- 主要OS/アプリの致命的な衝突を回避（Ctrl+SpaceやAlt+Spaceの既知衝突を回避）
- 複合キーとしての誤起動耐性が高い
...
1. **Ctrl \+ Shift \+ Space（最優先推奨）**  
   * **理由**：スペース文字の副作用を回避しつつ、スクリーンリーダー非競合かつOS/主要アプリとの衝突が最小。誤起動耐性が高く、WCAG 2.1の観点でも安全側。  
2. **Shift \+ Space（2キーの次点・副作用あり）**  
   * **理由**：2キー制約下では最良だが、実機で「押下中にスペースが継続挿入」される副作用が確認され、デフォルト用途には不適。文脈限定でのカスタム用途は可。  
3. **Ctrl \+ Backslash (\\)**  
   * **理由**：競合リスクは低いが、レイアウト差の影響が大きく操作一貫性に欠ける。  
4. **Ctrl \+ Space**  
   * **理由**：macOSやIDEと競合するため、デフォルトとしては不適。  
5. **Alt \+ Space**  
